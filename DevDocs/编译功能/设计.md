...
跨平台协作时，Mac/Linux 提交的源码文件可能使用 LF (`\n`) 或 CR (`\r`) 换行符，导致 Windows 编译失败。在调用编译命令前自动修复。

**扫描目录**：
- `项目根/Source/`
- `项目根/ThirdParty/`
- `项目根/Plugins/` 下递归查找所有 `Source/` 和 `ThirdParty/` 子目录
...
- 只读文件临时去除只读属性，修复后恢复
- 单文件异常不中断整体流程
- 不扫描 `Intermediate/`、`Content/` 等无关目录（白名单模式）

**不需要处理的文件**：
| 文件类型 | 原因 |
|---------|------|
| `.uasset` / `.umap` | 二进制格式，不涉及换行符 |
| `.uproject` / `.uplugin` | UBT 使用 `System.Text.Json` 解析，对换行符不敏感 |
| `.ini` | UE 配置解析器兼容 LF/CRLF |
...
| 跨平台换行符导致编译失败 | 编译前自动扫描修复源码文件换行符为 CRLF |
| 插件目录嵌套层级不固定 | Plugins/ 下递归查找 Source/ 和 ThirdParty/ |
